...
Step 2: ETL (Extract, Transform, Load) Pipeline
...
-- CTE 1: Clean up the `raw_accounts` table
dim_users AS (
    SELECT
        account_id,
        -- We assume `signup_date` from `raw_accounts` is the source of truth
...
first_week_usage AS (
    ...
    FROM
        raw_feature_usage AS fu
    -- Join with our user dimension to get their signup date
    JOIN
        dim_users AS u ON fu.user_id = u.account_id
    -- This is the key: only look at events from their first 7 days
    WHERE
        DATETIME(fu.event_timestamp) BETWEEN u.signup_date AND DATE(u.signup_date, '+7 days')
    GROUP BY
        fu.user_id
),
...
first_week_tickets AS (
    ...
    FROM
        raw_support_tickets AS st
    JOIN
        dim_users AS u ON st.user_id = u.account_id
    WHERE
        -- Only count tickets from their first 7 days
        DATETIME(st.created_at) BETWEEN u.signup_date AND DATE(u.signup_date, '+7 days')
    GROUP BY
        st.user_id
),

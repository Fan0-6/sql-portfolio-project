...
1.  **Extract:** Selects data from the 5 raw tables (`ravenstack_accounts`, `ravenstack_subscriptions`, etc.).
2.  **Transform:**
...
3.  **Load:** Creates the final `fct_user_summary` table, which serves as our fast, clean data source for all analysis.
...
| user_group | total_users | avg_collab_used | avg_reports_used | avg_admin_used | avg_support_tickets |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Non-Converted** | 281 | 0.10 | 0.02 | 0.11 | 0.06 |
| **Converted** | 219 | **0.23** | **0.08** | 0.04 | 0.04 |
...
| collab_usage_bucket | total_users_in_bucket | total_conversions | conversion_rate_percent |
| :--- | :--- | :--- | :--- |
| `0_uses` | 491 | 213 | **43.38%** |
| `3_or_more_uses` | 9 | 6 | **66.67%** |
...
1.  **Redesign Onboarding:** The new-user onboarding experience must funnel all users directly into the **'Collaboration'** and **'Reports'** features.
2.  **Set a Goal:** The Product team's #1 goal should be to get every new user to use the 'Collaboration' feature **at least 3 times** in their first week, as this makes them **1.5x more likely to convert.**
3.  **Reduce Friction:** The 'Admin' feature should be de-emphasized, and the high number of support tickets from non-converters should be investigated to reduce confusion.
